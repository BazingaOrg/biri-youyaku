...
- **历史页用量与周报**：Token / 费用统计与周报摘要都在历史页内（非独立 `/stats` 路由）。
...
- 本机一体：`docker compose up --build`（Web `5173`，API `17821`）
