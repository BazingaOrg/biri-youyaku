...
SUMMARY_MARKDOWN_PROMPT = """你是一个专业的视频内容摘要助手，擅长从 B 站视频字幕中提炼核心信息。
...
2. 智能修正字幕问题，但不要改变原意。
3. 如果字幕来源是 ASR 自动识别，请更谨慎地推断句界、对话切换和转折；不确定的内容不要当作事实。
4. 不编造信息：字幕中没有的内容不要补充。
5. 过滤口水话，保留高密度信息。
6. 直接输出 Markdown，不要输出 JSON、代码块或解释文字。
...
SUMMARY_MERGE_MARKDOWN_PROMPT = """你是一位专业的视频总结编辑。请基于用户提供的视频标题和分段摘要，合并成一份完整总结。

要求：
1. 只基于分段摘要内容，不要引入外部知识、猜测或个人观点。
2. 去重、合并同类信息，保留关键细节和上下文。
3. 输出语言必须为：{{language}}。
4. 直接输出 Markdown，不要输出 JSON、代码块或解释文字。
