...
export type KnowledgeChatStreamEvent = 'status' | 'delta' | 'citations' | 'done' | 'error'

export interface KnowledgeChatStreamMessage {
  event: KnowledgeChatStreamEvent
  data: string
}

/** POST SSE for knowledge chat (query body, not GET EventSource). */
export function openKnowledgeChatStream(
  body: {query: string; limit?: number},
  onMessage: (message: KnowledgeChatStreamMessage) => void,
  onError: (error: Error) => void,
  onClose?: () => void,
): JobStreamSubscription {
  const controller = new AbortController()

  void readPostStream('/v1/knowledge/chat', body, controller.signal, onMessage)
    .catch((error) => {
      if (!controller.signal.aborted) {
        onError(error instanceof Error ? error : new Error('知识问答流失败'))
      }
    })
    .finally(() => {
      if (!controller.signal.aborted) {
...
async function readPostStream<TMessage extends {event: string; data: string}>(
  path: string,
  body: unknown,
  signal: AbortSignal,
