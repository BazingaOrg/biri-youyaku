...
> **2026-07 修订（A2 / video-only）**：蒸馏流水线改为仅投稿视频；动态抓取、
> 清洗、`GET /v1/up/{mid}/dynamics`、`dynamic.py` 已移除。遗留 `dynamics.md`
> / 旧 `dynamics_status` 列不自动删除；新 run 不再写入动态状态。

## 目标与边界

一键为某 UP 主生成「蒸馏语料包」：补齐投稿转写 → 逐视频观点提取 → 组装为
`data/distill/<mid>/` 目录。**本项目到语料包为止**，蒸馏 skill（SKILL.md 人物化）
在项目外生成，不在范围内。

Not building：SKILL.md 生成、蒸馏链路的邮件通知、批评者视角补充搜索、
现有笔记总结行为的改动（除 prompts 的两行增益性原则）、动态抓取/清洗。

## 关键决策

1. **分层 map-reduce**：视频逐个提取（转写超上下文 + 可断点续跑）；末端只组装
   不做有损压缩（压缩是项目外蒸馏阶段的职责）。
...
5. **从严默认**：视频上限默认 50（UI 可改）。防「一键跑两天 + 账号风控」。

## 接口与数据形态

- 投稿列表：复用 space API / `GET /v1/up/{mid}/videos`。
- 蒸馏 API：`POST /v1/up/{mid}/distill`（video_limit 默认 50）、状态查询、
  SSE 进度、结果读取。重启后按 `distill_runs` + 已有文件断点续跑。
- 产物目录：
  ```
  data/distill/<mid>/
    manifest.json      # 作者信息、数量、时间范围、per-video 状态
    videos/<bvid>.md   # 每视频观点提取（frontmatter：标题/日期/播放量）
    corpus.md          # 组装后的单文件语料包（video-only）
  ```
  （历史目录中若仍有 `dynamics.md`，不自动删除；新组装不读取。）
- 提取输出固定四节：观点与立场（附支撑原话）/ 思维方式 / 价值倾向 /
  低观点密度标记（纯教程类降权用）。
- prompts 位置：`modules/llm/distill_prompts.py`（DISTILL_EXTRACT_PROMPT、
  DISTILL_EXTRACT_MERGE_PROMPT），与笔记 prompts 分离。
...
| 2 | `_guard.py` 提取（历史：曾含 dynamics 路由，现已移除） | 回归 |
| 3 | distill 包 + distill_prompts + 迁移 + 路由 + 续跑 | 小体量作者端到端（含复用与重启续跑） |
| 4 | UpPage 蒸馏按钮/弹窗/SSE 进度/结果预览 | tsc + build + 浏览器端到端 |
| A2 | video-only：移除动态阶段与公开 dynamics API | distill 相关 pytest + app 导入 |
